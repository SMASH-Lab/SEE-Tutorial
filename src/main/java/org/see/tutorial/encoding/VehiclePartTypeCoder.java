...
public class VehiclePartTypeCoder implements Coder<VehiclePartType> {
    private final HLAinteger16BE coder;

    public VehiclePartTypeCoder() {
        EncoderFactory encoderFactory = HLAUtilityFactory.INSTANCE.getEncoderFactory();
        this.coder = encoderFactory.createHLAinteger16BE();
    }

    @Override
    public VehiclePartType decode(byte[] buffer) throws DecoderException {
        coder.decode(buffer);
        return VehiclePartType.fromValue(coder.getValue());
    }

    @Override
    public byte[] encode(VehiclePartType vehiclePartType) {
        coder.setValue(vehiclePartType.getValue());
        return coder.toByteArray();
    }

    @Override
    public Class<VehiclePartType> getAllowedType() {
        return VehiclePartType.class;
    }
}
